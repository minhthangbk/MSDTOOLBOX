...
% Intialization for some variablles in the function
    wi = [];
    filter_lengths_rg = []; %regular filter
...
% Design decimator filters in a regular FIR structure
    ...
    for j = 1 : data.K
        if j == 1 %for the first stage
            [filter(j).n, filter(j).fo, filter(j).ao, filter(j).w] = remezord([Fp(j) Fc(j)], [1 0], [Rp(j) Rc(j)], data.Fs);%approximate order n
            
            %optimal FIR filter design based on approximated values
            if nargin == 11 & mb_taps_lengths ~= 0          
                filter_stages_rg(j).b  = remez(mb_taps_lengths(j), filter(j).fo, filter(j).ao, filter(j).w);%with desired tap lengths
            else
                filter_stages_rg(j).b  = remez(filter(j).n, filter(j).fo, filter(j).ao, filter(j).w);%with MATLAB default tap lengths
            end
        else %for the up comming stages (Fs_k+1 = Fs/(M_1xM_2x...M_k))
            ...
            filter_lengths_rg(j) = length(filter_stages_rg(j).b); %get filter length
            filter_coefficients_rg(j, 1:filter_lengths_rg(j))=filter_stages_rg(j).b; %get coefficients
    end
    ...
    HB_Sinc_Filter(P)=.5;
    thewin=window(@hamming, N); %returns the n-point Hamming window
    HB_Window = HB_Sinc_Filter .* thewin';
    HB_Filter = HB_Window/sum(HB_Window); %normalized
